...
import { Music, Calendar, MapPin, ArrowLeft, Loader2, Plus, Check } from 'lucide-react';
...
const EventPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const body = type === 'setlist' ? { action, setlistId: eventId } : { action, venueId: eventId, year };
        ...
        {loading ? <Loader2 className="animate-spin mx-auto h-8 w-8 text-primary"/> : (
          <>
            ...
            <p className="text-muted-foreground flex items-center gap-2 mb-8">
              <Calendar className="h-4 w-4"/> {data?.setlist?.eventDate || year}
            </p>

            {type === 'setlist' ? (
              <div className="bg-card p-6 rounded-xl border">
                <h3 className="text-xl font-display mb-4">Setlist</h3>
                <ul className="space-y-2 mb-8">
                  {data?.setlist?.songs.map((s: string, i: number) => (
                    <li key={i} className="flex gap-4 p-2 hover:bg-muted rounded-lg">
                      <span className="text-primary font-bold">{i+1}</span> {s}
                    </li>
                  ))}
                </ul>
                <Button variant="fire" className="w-full" onClick={() => navigate('/generate')}>Générer ma playlist</Button>
              </div>
            ) : (
              <div className="grid gap-4">
                {data?.artists?.map((a: any, i: number) => (
                  <div key={i} className="bg-card p-4 rounded-xl border flex justify-between items-center">
                    <div onClick={() => navigate(`/event/${a.setlistId}?type=setlist`)} className="cursor-pointer">
                      <p className="font-bold text-lg">{a.name}</p>
                      <p className="text-sm text-muted-foreground">{a.eventDate}</p>
                    </div>
                    <Button 

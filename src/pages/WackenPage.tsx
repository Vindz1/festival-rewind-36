...
// On importe depuis la racine de /data/ pour éviter le crash Vercel
import { WACKEN_LINEUP, ArtistData } from '@/data/wackenData';
import { Footer } from '@/components/Footer';

const WackenPage = () => {
  const navigate = useNavigate();
  const [artists] = useState<ArtistData[]>(WACKEN_LINEUP);
  ...
  const handleGenerate = () => {
    ...
    const selectedArray = artists
      ...
      .map(a => ({
        id: a.id,
        artist: a.name,
        eventDate: a.day + " - Wacken Open Air 2026"
      }));

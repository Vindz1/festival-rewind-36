...
export default function Index() {
  ...
  const handleSetlistConnect = async (e: React.FormEvent) => {
    ...
    if (uname) {
      ...
      // 1. Sauvegarde en local (sécurité supplémentaire)
      localStorage.setItem('setlist_username', uname);
      localStorage.setItem('setlistUsername', uname);
      
      // 2. Mise à jour officielle dans ton profil Supabase si tu es connecté
      if (user) {
        try {
      ...
      setIsLinking(false);
      // 3. Redirection forcée (window.location force MyConcerts à recharger les données fraîches)
      window.location.href = `/my-concerts?username=${encodeURIComponent(uname)}`;
    } else {
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            >
              ...
              <div className="relative">
                ...
                <p className="text-gray-400 mb-6 leading-relaxed">
                  Retrouvez vos concerts vécus grâce à l'immense base de setlist.fm. Revivez l'ambiance, titre par titre.
                </p>
                <ul className="space-y-3 mb-8">
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#4d94ff]" /> Recherche par artiste
                  </li>
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#4d94ff]" /> Setlists exactes jouées en concert
                  </li>
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#4d94ff]" /> 2 exports gratuits par an
                  </li>
                </ul>
                <div className="flex items-center gap-2 text-[#4d94ff] font-bold group-hover:gap-4 transition-all">
                  <span>Voir mes concerts</span><span>→</span>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="relative">
                ...
                <ul className="space-y-3 mb-8">
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500" /> Hellfest, Download UK, Wacken...
                  </li>
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500" /> Carte interactive mondiale
                  </li>
                  <li className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500" /> Nouvelles programmations ajoutées régulièrement
                  </li>
                </ul>
                <div className="flex items-center gap-2 text-green-500 font-bold group-hover:gap-4 transition-all">
                  <span>Découvrir les festivals</span><span>→</span>
                </div>
              </div>
            </div>
      ...
      <section className="relative z-10 py-20 bg-[#0a0a0a]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-400 text-lg">De la découverte à l'expérience illimitée</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-[#2d2d2d] to-[#1a1a1a] border border-[#404040] rounded-2xl p-6 sm:p-8 hover:border-gray-500 transition-colors">
              ...
              <div className="flex items-baseline gap-1 mb-4"><span className="text-3xl font-black">Lecture</span></div>
              <p className="text-sm text-gray-400 mb-6">Pour explorer le site</p>
              <ul className="space-y-3 mb-8">
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-gray-400">✓</div><span>Consulter les festivals</span></li>
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-gray-400">✓</div><span>Voir les programmations</span></li>
                <li className="flex items-start gap-3 text-sm text-gray-500"><div className="w-5 h-5 shrink-0">✗</div><span>Pas d'export possible</span></li>
                <li className="flex items-start gap-3 text-sm text-gray-500"><div className="w-5 h-5 shrink-0">⚠</div><span>Avec publicités</span></li>
              </ul>
              <Button variant="outline" className="w-full h-11 border-[#404040] text-gray-400 cursor-default" disabled>Mode actuel</Button>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#2d2d2d] to-[#1a1a1a] border border-[#404040] rounded-2xl p-6 sm:p-8 hover:border-white transition-colors">
              ...
              <div className="flex items-baseline gap-1 mb-4">
                <span className="text-3xl font-black">0€</span><span className="text-gray-400 text-sm">/à vie</span>
              </div>
              <p className="text-sm text-gray-400 mb-6">Pour les festivaliers occasionnels</p>
              <ul className="space-y-3 mb-8">
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-white">✓</div><span className="text-white"><strong>2 exports par an</strong></span></li>
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-gray-400">✓</div><span>Prévisualisation</span></li>
                <li className="flex items-start gap-3 text-sm text-gray-500"><div className="w-5 h-5 shrink-0">⚠</div><span>Avec publicités</span></li>
              </ul>
              <Button onClick={() => navigate('/auth')} className="w-full h-11 bg-[#333] hover:bg-[#444] text-white font-bold transition-colors">Créer un compte</Button>
            </div>
            ...
            <div className="relative bg-gradient-to-br from-[#2d2d2d] to-[#1a1a1a] border-2 border-[#4d94ff] rounded-2xl p-6 sm:p-8 shadow-[0_0_30px_-10px_rgba(77,148,255,0.3)] hover:shadow-[0_0_40px_-10px_rgba(77,148,255,0.5)] transition-all">
              <div className="absolute top-4 right-4 bg-[#4d94ff] text-white text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-widest shadow-md">Recommandé</div>
              <h3 className="text-xl font-bold mb-2 flex items-center gap-2">Premium<Crown className="w-5 h-5 text-yellow-500" /></h3>
              <div className="flex items-baseline gap-1 mb-4"><span className="text-3xl font-black text-[#4d94ff]">5€</span><span className="text-gray-400 text-sm">/an</span></div>
              <p className="text-sm text-gray-400 mb-6">L'expérience ultime</p>
              <ul className="space-y-3 mb-8">
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-[#4d94ff]">✓</div><span><strong className="text-[#4d94ff]">Exports ILLIMITÉS</strong></span></li>
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-[#4d94ff]">✓</div><span>Zéro publicité</span></li>
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-[#4d94ff]">✓</div><span>Historique complet</span></li>
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-[#4d94ff]">✓</div><span>Badge supporter</span></li>
                <li className="flex items-start gap-3 text-sm"><div className="w-5 h-5 shrink-0 text-[#4d94ff]">✓</div><span>Support prioritaire</span></li>
              </ul>
              <Button onClick={() => navigate('/subscription')} className="w-full h-11 bg-[#4d94ff] hover:bg-[#6ba6ff] text-white font-bold shadow-md hover:shadow-lg transition-all">Devenir Premium</Button>
            </div>
      ...
      <section className="relative z-10 py-20 bg-gradient-to-b from-[#1a1a1a] to-black">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-3xl sm:text-5xl font-black italic uppercase mb-6">
            PRÊT À REVIVRE<br /><span className="text-[#4d94ff]">VOS CONCERTS ?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button onClick={() => navigate('/my-concerts')} size="lg" className="bg-[#4d94ff] hover:bg-[#6ba6ff] text-white font-bold px-10 h-14 text-lg rounded-full shadow-lg shadow-blue-500/30 transition-transform active:scale-95">
              Commencer maintenant
            </Button>
            <Button onClick={() => navigate('/festivals')} size="lg" variant="outline" className="bg-transparent hover:bg-white/10 text-white border-white/30 font-bold px-10 h-14 text-lg rounded-full transition-transform active:scale-95">

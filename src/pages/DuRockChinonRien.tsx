...
import { Filter, Check, XCircle, ChevronDown, ChevronUp, ExternalLink, Ticket } from 'lucide-react';
import { toast } from 'sonner';
import { DU_ROCK_CHINON_RIEN_LINEUP, ArtistData } from '@/data/DuRockChinonRien'; 
import { Footer } from '@/components/Footer';

const DuRockChinonRienPage = () => {
  const navigate = useNavigate();
  const [artists] = useState<ArtistData[]>(DU_ROCK_CHINON_RIEN_LINEUP);
  ...
  const handleGenerate = () => {
    ...
    const playlistData = {
      playlistName: `Du Rock Chinon Rien - My Selection`,
      mainArtist: 'Du Rock Chinon Rien',
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 sm:pt-32 pb-8">
        ...
        <div className="mb-8">
          <h1 className="text-4xl sm:text-6xl font-black italic uppercase tracking-tighter mb-4 text-white drop-shadow-lg">
            SOIRÉE <br/>
            <span className="text-[#4d94ff]">DU ROCK CHINON RIEN</span>
          </h1>
          <p className="text-lg text-gray-300 font-medium mb-4">
            Samedi 4 Avril 2026 • Espace Rabelais (Chinon)
          </p>

          <div className="flex flex-wrap gap-3">
            <a 
              href="https://swll.to/WJoUHe" 
              target="_blank" 
            ...
            <a 
              href="https://swll.to/FyUrk8F" 
              target="_blank" 
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#4d94ff]/10 hover:bg-[#4d94ff]/20 border border-[#4d94ff]/30 rounded-full text-sm font-bold text-[#4d94ff] transition-all backdrop-blur-sm"
            >
...
export default DuRockChinonRienPage;

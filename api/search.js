export default async function handler(req, res) {
  const { action, name, city, year, venueId, setlistId, p = 1 } = req.query;
  const apiKey = process.env.SETLIST_FM_API_KEY;
  const headers = { 'x-api-key': apiKey, 'Accept': 'application/json', 'User-Agent': 'Mozilla/5.0' };
  const sleep = (ms) => new Promise(r => setTimeout(r, ms));

  try {
    // 1. RECHERCHE DE FESTIVALS (Action: searchFestivals)
    if (action === 'searchFestivals') {
      const resultsMap = new Map();
      // On scanne 3 pages (suffisant avec les filtres Ville/Année)
    ...
    // 2. RECHERCHE D'ARTISTES (Action: searchArtists)
    ...
    // 3. LISTE DES ARTISTES (Action: getFestivalArtists)
    ...
    // 4. RÉCUPÉRATION DES MORCEAUX (Action: getSetlist)
    if (action === 'getSetlist') {
      const response = await fetch(`https://api.setlist.fm/rest/1.0/setlist/${setlistId}`, { headers });
      if (!response.ok) return res.status(404).json({ error: "Setlist non trouvée" });
